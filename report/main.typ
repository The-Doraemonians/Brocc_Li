...
The pursuit of a healthy lifestyle is a universal goal, yet many individuals struggle to maintain a balanced diet. The reasons are multifaceted, ranging from a lack of knowledge about nutrition to the difficulty of balancing dietary needs with budget, convenience, and personal taste. Existing solutions often provide generic advice that fails to account for an individual's unique circumstances, such as specific health goals (e.g., weight loss, muscle gain), dietary restrictions (e.g., vegetarian, gluten-free), and financial constraints. This creates a clear need for an accessible, affordable, and user-friendly diet management solution @el2013systematic that can deliver truly personalized guidance.

To address this problem, we developed Brocc Li, a personalized diet management companion. The project's central objective was to create an intelligent agent @yao2023react capable of generating tailored diet plans that are both nutritionally sound and practical for the user. By leveraging the advanced natural language understanding and generation capabilities of modern Large Language Models (LLMs) @brown2020language, Brocc Li aims to act as a personal diet assistant @laranjo2018conversational, simplifying the complex process of meal planning and grocery shopping .
...
The agent was built using a state-graph framework @mavroudis2024langchain, with the Gemini-2.5-flash model @team2023gemini from Google serving as the core reasoning engine @wei2022chain. The agent's main role is to interpret the user's request, decide which tool (or sequence of tools) is needed to fulfill it, and execute them accordingly.
...
*Web Scraping Tools*: A set of functions responsible for dynamically searching for and extracting recipe information and grocery prices from various online sources @liu2007web. This was a critical but challenging component due to the inconsistent structure of websites.
...
table(
  ...
  table.header[*Meal*][*Dish*][*Calories*],
...
#heading(numbering: none)[Future Work]
